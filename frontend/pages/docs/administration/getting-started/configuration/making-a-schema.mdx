...
`server/scripts/example_schemas`. It is important to note that these schemas contain all of mandatory fields that the
application needs in order to run. These fields are hard-coded in various places in the source code.
...
Mongo directly, or, by using the addUploadSchemas.ts / addDeploymentScript.ts scripts found within `server/scripts`.
This process is not currently very graceful and requires some minor editing of the scripts themselves.
